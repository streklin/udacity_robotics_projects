...
static float CenterBoxDistance(const math::Box& a, const math::Box& b) {
	float b1_z = ((b.max.z - b.min.z) / 2) + b.min.z;
  	float b1_y = ((b.max.y - b.min.y) / 2) + b.min.y;
  	float b1_x = ((b.max.x - b.min.x) / 2) + b.min.x;
  
  	float a1_z = ((a.max.z - a.min.z) / 2) + a.min.z;
  	float a1_y = ((a.max.y - a.min.y) / 2) + a.min.y;
  	float a1_x = ((a.max.x - a.min.x) / 2) + a.min.x;
  
  	float sqrDist = 0;
  	sqrDist += (a1_z - b1_z) * (a1_z - b1_z);
  	sqrDist += (a1_x - b1_x) * (a1_x - b1_x);
  	sqrDist += (a1_y - b1_y) * (a1_y - b1_y);
...
bool ArmPlugin::createAgent()
{
	if( agent != NULL )
  	...
  	// NOTES:
  	// num actions = DOF * 2, can move either in positive or negative direction
    // in each degree of freedom. For example, arm can rotate either up or down.
...
void ArmPlugin::onCollisionMsg(ConstContactsPtr &contacts)
{
	...
	for (unsigned int i = 0; i < contacts->contact_size(); ++i)
	{
        ...
        // bool isGripperCollision = (strcmp(contacts->contact(i).collision2().c_str(), COLLISION_POINT) == 0);
      	bool isGripperCollision = (contacts->contact(i).collision2().find("gripper") != std::string::npos);
      
      /*
		// Used for full arm collision task
        if ( isArmCollision || isGripperCollision ) {
            if (DEBUG) std::cout << "Issue Reward for ARM COLLISION \n";
        ...
        */
      	if ( isGripperCollision ) {
        	if (DEBUG) std::cout << "Issue Reward for GRIPPER COLLISION \n";
          	int score = maxEpisodeLength - episodeFrames;
          
            rewardHistory = REWARD_WIN + score;
			newReward  = true;
			endEpisode = true;
        } else if ( isArmCollision ) {
         	if (DEBUG) std::cout << "Issue NEGATIVE REWARD for ARM COLLISION \n";
          
            std::cout << "ISSUE REWARD FOR ARM \n";
...
void ArmPlugin::OnUpdate(const common::UpdateInfo& updateInfo)
{
	...
	if( maxEpisodeLength > 0 && episodeFrames > maxEpisodeLength )
	{
		...
		rewardHistory = 2*REWARD_LOSS; // NO SLACKING!
		newReward     = true;
		endEpisode    = true;
	}
	...
	if( hadNewState && !newReward )
	{
		...
		const float groundContact = 0.01f; // keeping it at the previous level seems to result in nonsense solutions
	
      	bool checkGroundContact = false;
      
      	// when am I hitting the ground?
      	// its a bounding box, so presumably oriented by z
        // so if z max or z min is less than groundContact we can
        // reasonably assume we are touching the ground.
      	// box.max.x
        if (gripBBox.max.z <= groundContact || gripBBox.min.z <= groundContact) checkGroundContact = true;
		...
		{		
			if(DEBUG){printf("GROUND CONTACT, EOE\n");}
          	...
          	std::cout << "ISSUE REWARD FOR GROUND \n";
          	rewardHistory 	= REWARD_LOSS + (-1) * REWARD_LOSS * exp(-distGoal); 
          	endEpisode 		= true;
          	newReward 		= true;
          	
          
            if(DEBUG) std::cout << "Ground Reward: " << rewardHistory << "\n";
		}
		...
		if(!checkGroundContact)
		{
			...
			if(DEBUG){printf("distance('%s', '%s') = %f\n", gripper->GetName().c_str(), prop->model->GetName().c_str(), distGoal);}
          	...
          	if (DEBUG) { 
              std::cout << "Bounding X: " << gripBBox.max.x << "\n"; 
              std::cout << "PROP X: " <<  propBBox.max.x << "\n";
            }
          
			if( episodeFrames > 1 )
			{
                //rewardHistory = REWARD_LOSS + (-1) * REWARD_LOSS * exp(-distGoal);//110.0f * exp(-distGoal);
              	float delta = lastGoalDistance - distGoal;
              	avgGoalDelta  = (avgGoalDelta * DISTANCE_ALPHA) + (delta * (1 - DISTANCE_ALPHA));	
              	rewardHistory = avgGoalDelta;
               	newReward     = true;	
              	if(DEBUG) std::cout << "Interim Reward: " << rewardHistory << "\n";
                //std::cout << "Interim Reward: " << rewardHistory << "\n";
